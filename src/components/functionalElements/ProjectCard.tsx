              ...
              {project.imagesURLs.map((_, i) => (
                <div
                  ...
                  className={`h-[6px] rounded-full transition-all duration-300 ${
                    i === currentImgIndex
                      ? "w-[16px] bg-[#008CFF] shadow-[0_0_8px_rgba(0,140,255,0.8)]"
                      : "w-[6px] bg-[#F5F5F5]/40"
                  }`}
                />
              ))}
      ...
      <div className="flex flex-col flex-1 p-[16px]">
        ...
        <div className="w-full flex flex-wrap gap-2 mt-4">
          {project.tags.map((tag, index) => (
            <div
              key={index}
              className="px-2.5 py-1 bg-[#008CFF]/10 border border-[#008CFF]/20 rounded-[6px] text-[#008CFF] text-[11px] font-medium tracking-wide"
            >
              {tag.toUpperCase()}
            </div>
          ))}
